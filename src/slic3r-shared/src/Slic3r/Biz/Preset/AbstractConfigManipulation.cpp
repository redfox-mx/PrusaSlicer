...
void AbstractConfigManipulation::update_print_fff_config(DynamicPrintConfig* config, const DynamicPrintConfig* initial_config, const bool is_global_config)
{
    ...
    if (config->opt_float("layer_height") < Domain::EPSILON)
    {
        ...
        warn_user(_u8L("Layer height"), _u8L("Layer height is not valid.\n\nThe layer height will be reset to 0.01."));
    ...
    if (config->option<ConfigOptionFloatOrPercent>("first_layer_height")->value < Domain::EPSILON)
    {
        ...
        warn_user(_u8L("First layer height"), _u8L("First layer height is not valid.\n\nThe first layer height will be reset to 0.01."));
    ...
    {
        std::string msg_text = _u8L("The Spiral Vase mode requires:\n"
                                "- one perimeter\n"
                                "- no top solid layers\n"
                                "- 0% fill density\n"
                                "- no support material\n"
        ...
        if (is_global_config)
            msg_text += "\n\n" + _u8L("Shall I adjust those settings in order to enable Spiral Vase?");

        DynamicPrintConfig new_conf = *config;

        bool answer{ false };
        if (is_global_config)
            answer = ask_user(_u8L("Spiral Vase"), msg_text);
        else 
            warn_user(_u8L("Spiral Vase"), msg_text);

        bool support = true;
        if (!is_global_config || answer) {
    ...
    if (config->opt_bool("wipe_tower") && config->opt_bool("support_material") && 
        ...
        if (config->opt_float("support_material_contact_distance") == 0) {
            if (!config->opt_bool("support_material_synchronize_layers")) {
                std::string msg_text = _u8L("For the Wipe Tower to work with the soluble supports, the support layers\n"
                                        "need to be synchronized with the object layers.");
                if (is_global_config)
                    msg_text += "\n\n" + _u8L("Shall I synchronize support layers in order to enable the Wipe Tower?");

                DynamicPrintConfig new_conf = *config;

                bool answer{ false };
                if (is_global_config)
                    answer = ask_user(_u8L("Wipe Tower"), msg_text);
                else
                    warn_user(_u8L("Wipe Tower"), msg_text);
            ...
            if ((config->opt_int("support_material_extruder") != 0 || config->opt_int("support_material_interface_extruder") != 0)) {
                std::string msg_text = _u8L("The Wipe Tower currently supports the non-soluble supports only "
                                        "if they are printed with the current extruder without triggering a tool change. "
                                        "(both support_material_extruder and support_material_interface_extruder need to be set to 0).");
                if (is_global_config)
                    msg_text += "\n\n" + _u8L("Shall I adjust those settings in order to enable the Wipe Tower?");
    ...
    if (is_global_config && config->opt_bool("support_material")) {
        ...
        if (!m_support_material_overhangs_queried) {
            ...
            if (!config->opt_bool("overhangs")/* != 1*/) {
                std::string msg_text = _u8L("Supports work better, if the following feature is enabled:\n"
                                        "- Detect bridging perimeters");
                if (is_global_config)
                    msg_text += "\n\n" + _u8L("Shall I adjust those settings for supports?");

                DynamicPrintConfig new_conf = *config;

                if (ask_user(_u8L("Support Generator"), msg_text)) {
    ...
    if (config->option<ConfigOptionPercent>("fill_density")->value == 100) {
        ...
        if (bool correct_100p_fill = config->option_def("top_fill_pattern")->enum_def->enum_to_index(fill_pattern).has_value(); 
            ...
            if (auto label = fill_pattern_def->enum_def->enum_to_label(fill_pattern); label.has_value()) {
                const std::string label_str = _(*label);
                std::string msg_text = Slic3r::format(_u8L("The %1% infill pattern is not supposed to work at 100%% density."), label_str);
                if (is_global_config)
                    msg_text += "\n\n" + _u8L("Shall I switch to rectilinear fill pattern?");

                DynamicPrintConfig new_conf = *config;

                bool answer{ false };
                if (is_global_config)
                    answer = ask_user(_u8L("Infill"), msg_text);
                else
                    warn_user(_u8L("Infill"), msg_text);
                ...
                if (!is_global_config || answer) {
                    ...
                    fill_density = initial_config->option<ConfigOptionPercent>("fill_density")->value;
...
void AbstractConfigManipulation::toggle_print_fff_options(DynamicPrintConfig* config)
{
    ...
    bool have_brim = config->opt_enum<Domain::BrimType>("brim_type") != Domain::BrimType::NoBrim;
...
void AbstractConfigManipulation::toggle_print_sla_options(DynamicPrintConfig* config)
{
